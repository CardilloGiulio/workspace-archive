...
export function loadObjectsFromJsonFolder(): SceneObjectData[] {
  ...
  if (jsonFiles.length === 0) {
    debugWarn("objectLoader", "No JSON files found in src/json/scene-objects/");
    return [];
  }
...
function isSceneObjectData(value: unknown): value is SceneObjectData {
  ...
  const objectType = value.objectType ?? "primitive";

  if (!isObjectType(objectType)) {
    return false;
  }

  if (objectType === "primitive" && !isShapeType(value.shape)) {
    return false;
  }

  if (objectType === "model" && typeof value.modelKey !== "string") {
    return false;
  }

  if (!isOptionalString(value.parentGroup)) return false;
  ...
  if (!isOptionalVector3Tuple(value.scale)) return false;
  if (!isOptionalVector3Tuple(value.color)) return false;
  if (!isOptionalVector3Tuple(value.debugColor)) return false;
  ...
  if (!isOptionalNumber(value.visibility)) return false;
  if (!isOptionalNumber(value.alpha)) return false;
